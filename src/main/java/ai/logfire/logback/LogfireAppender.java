...
public class LogfireAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
  ...
  public LogfireAppender() {
    ...
    scheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(new LogfireSender(), batchInterval, batchInterval,
  ...
  protected void append(ILoggingEvent event) {
    ...
    if (this.ingestUrl.isEmpty() || this.sourceToken == null || this.sourceToken.isEmpty()) {
      // Prevent potential deadlock, when a blocking logger is configured - avoid
      // using logger directly in append
      startThread("logfire-warning-logger", () -> {
        logger.warn(
            "Missing Source token for Logfire - disabling LogfireAppender. Find out how to fix this at: https://logfire.ai/docs/logs/java ");
      });
      this.disabled = true;
      return;
    }
  ...
  /**
   * Sets the MDC fields types that will be sent as metadata, in the same order as
   * <i>mdcFields</i> are set
   * up, separated by a comma. Possible values are <i>string</i>, <i>boolean</i>,
   * <i>int</i> and <i>long</i>.
   *
   * @param mdcTypes
   *                 MDC fields types
   */
  ...
  /**
   * Sets the maximum number of messages in the queue. Messages over the limit
   * will be dropped.
   *
   * @param maxQueueSize
  ...
  /**
   * Sets the maximum wait time for a batch to be sent via the API, in
   * milliseconds.
   *
   * @param batchInterval
  ...
  /**
   * Sets the maximum number of retries for sending logs to Logfire. After that,
   * current batch of logs will be dropped.
   *
   * @param maxRetries
  ...
  /**
   * Sets the number of milliseconds to sleep before retrying to send logs to
   * Logfire.
   *
  ...
  /**
   * Registers a dynamically loaded Module object to ObjectMapper used for
   * serialization of logged data.
   *
   * @param className
   *                  fully qualified class name of the module, eg.
